...
class ChargeCommandRequest(BaseModel):
    ...
    charge_start_time: str = Field(..., alias="chargeStartTime")
...
def actual_charge_process(request: ChargeCommandRequest):
    # robotId가 1일 때, 실제 충전 명령 로직을 처리하는 부분
    # 추후 로직 구현 예정
...
def fake_charge_process(request: ChargeCommandRequest):
    # robotId가 2일 때, 가짜 충전 명령 로직을 처리하는 부분
    # 추후 로직 구현 예정
...
async def charge_command(request: ChargeCommandRequest):
    ...
    if request.robot_id == 1:
        ...
        response = {"status": "error", "message": "Unknown robot ID", "data": request}
