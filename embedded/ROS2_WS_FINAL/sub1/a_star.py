...
class AStar(Node):
    ...
    def __init__(self):
        ...
        self.final_goal = [340, 122]
        self.intermediate_goal = [365, 170]
        
        self.car_goal = [51, 267]
        self.intermediate_car = [51, 220]

        # 맵의 해상도 설정 (한 그리드 셀당 0.05미터)
        self.map_resolution = 0.05

        # 맵의 오프셋 설정 (맵의 좌상단 기준, 맵의 물리적 위치를 결정하는 좌표 오프셋)
        self.map_offset_x = -2- 8.75
        self.map_offset_y = -2- 8.75
        ...
        self.timer = self.create_timer(3.0, self.timer_callback)  # 1.0초마다 콜백 실행
            ...
            for x in range(width):
                ...
                if self.grid[y][x] == 100:
                    for i in range(len(self.dx)):  # 8 방향을 순회
                        for distance in range(1, 3):  # 1칸, 2칸씩 탐색
    ...
    def goal_callback(self, msg):
        ...
        if msg.header.frame_id == 'map':
            ...
            if self.is_map == True and self.is_odom == True:
                ...
                print("start cell test : ", start_grid_cell)
                
                if self.goal == (340, 122):
                    isAstra = False
                    print("go ChargeTower!!")
                    self.astar_final(start_grid_cell)
                    isAstra = True
                elif self.goal == (51, 267):
                    isAstra = False
                    print("go Car!!")
                    self.astar_car(start_grid_cell)
                    isAstra = True
    ...
    def final_heuristic(self, node):
        # 특정 구역을 경유하도록 우선 특정 지점까지 가는 휴리스틱 계산
        waypoint = (365, 170)  # 특정 구역 내의 경유지점 설정
        
        # 현재 위치에서 경유지점까지의 맨해튼 거리 계산
        waypoint_dist = abs(node[0] - waypoint[0]) + abs(node[1] - waypoint[1])
        
        # 경유지점에서 목표까지의 맨해튼 거리 계산
        goal_dist_from_waypoint = abs(waypoint[0] - self.goal[0]) + abs(waypoint[1] - self.goal[1])
        
        # 노드가 경유지점을 지나기 전까지는 경유지점까지의 거리와 목표 거리 모두 고려
        if abs(node[0] - waypoint[0]) > 0 or abs(node[1] - waypoint[1]) > 0:
            return waypoint_dist + goal_dist_from_waypoint
        else:
            return goal_dist_from_waypoint
        
    def car_heuristic(self, node):
        # 특정 구역을 경유하도록 우선 특정 지점까지 가는 휴리스틱 계산
        waypoint = (55, 222)  # 특정 구역 내의 경유지점 설정
    ...
    def astar_car(self, start):
        print("astarCar!!!", flush=True)
        # 도착지 및 경유지 설정
        ...
        open_set = []
        heapq.heappush(open_set, (0 + self.car_heuristic(start), start))
        self.cost[start[1]][start[0]] = 0
        
        reached_car_intermediate = False  # 경유지 도달 여부
        ...
        while open_set:
            ...
            # 경유지에 도달했는지 확인
            if current == tuple(self.intermediate_car) and not reached_car_intermediate:
                print("Reached intermediate goal:", self.intermediate_car, flush=True)
                reached_car_intermediate = True  # 경유지 도달
                continue  # 경유지에 도달했으면 다음 노드 탐색으로 넘어감

            # 최종 목표에 도달했을 때
            if reached_car_intermediate and current == tuple(self.car_goal):
                print("Path found to final goal!", flush=True)
            ...
            for i in range(8):
                ...
                if (0 <= next_x < self.GRIDSIZE) and (0 <= next_y < self.GRIDSIZE):
                    if self.grid[next_y][next_x] < 50:  # 장애물 회피
                        new_cost = self.cost[current[1]][current[0]] + 1
                        if self.cost[next_y][next_x] > new_cost:
                            self.cost[next_y][next_x] = new_cost
                            priority = new_cost + self.final_heuristic(next_node)
                            heapq.heappush(open_set, (priority, next_node))
                            self.path[next_y][next_x] = current

        print("No path found to the final goal.", flush=True)

    
    def astar_final(self, start):
        print("astarFinal!!!", flush=True)
        # 도착지 및 경유지 설정
        ...
        open_set = []
        heapq.heappush(open_set, (0 + self.final_heuristic(start), start))
        self.cost[start[1]][start[0]] = 0
        
        reached_intermediate = False  # 경유지 도달 여부
        ...
        while open_set:
            ...
            # 경유지에 도달했는지 확인
            if current == tuple(self.intermediate_goal) and not reached_intermediate:
                print("Reached intermediate goal:", self.intermediate_goal, flush=True)
                reached_intermediate = True  # 경유지 도달
                continue  # 경유지에 도달했으면 다음 노드 탐색으로 넘어감

            # 최종 목표에 도달했을 때
            if reached_intermediate and current == tuple(self.final_goal):
                print("Path found to final goal!", flush=True)
                self.retrace_path(current)
                return
