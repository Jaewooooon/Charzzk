...
class odom(Node):
    ...
    def __init__(self):
        super().__init__('odom')
        print("Odom node is running ", flush = True)
        # self.get_logger().info("Odom node is running ")
    ...
    # IMU : 로봇의 헤딩(방향)을 실시간으로 추적하는 역할
    # 여기서 얻은 방향정보(theta)를 사용하여 로봇의 선속도, 각속도에 기반한 위치 계산을 수행
    def imu_callback(self,msg):
        #print("imucallback node is running ", flush = True)
        #pass
        '''
    ...
    def listener_callback(self, msg):
        #print("listner node is running ", flush = True)
        if self.is_status == False :
            self.is_status=True
            self.prev_time=rclpy.clock.Clock().now()
        else :
            #print("listenercallback node is running ", flush = True)
            #print('now_linear_vel_inOdom : {}    now_ angular_vel_inOdom : {}'.format(msg.twist.linear.x,-msg.twist.angular.z))
