...
# OCR 완료 후 충전 로직을 실행하는 함수 추가
def charging_process(jsondata):
    # 충전 클래스 정의 및 로직 수행
    class Charging:
...
class OCR(Node):
    ...
    def load_json_data(self):
        ...
        filename = os.path.join("C:\\Users\\SSAFY\\Desktop\\ws\\src\\sub1\\sub1", filename_txt)  # 절대 경로 설정
        try:
    ...
    def ocr(self, cropped_image, target_length = 7, max_attemps = 5):
        ...
        plt.axis('off')  # 축 숨기기
        plt.show(block=False)  # 비차단 모드로 이미지 표시
    ...
    def img_callback(self, msg):
        ...
        #     return
        if self.image_saved:
            print("Image already processed, skipping callback", flush=True)
            return
        ...
        if central_contours:
            ...
            if(picNumber == storedNumber):
                ...
                message = {
                    ...
                    "reservationId": jsondata['reservationId'],
                    "chargerId": jsondata['chargerId'],
                    "vehicleNumber": jsondata['carNumber'],
                    "timestamp": datetime.datetime.now().isoformat()
                    }
            ...
            print("message_pro : ", self.message_producer)
            if self.message_producer:
                ...
                # charge_ing로 실행 ㄱㄱ
                # OCR 완료 후 충전 로직 실행
                charging_process(jsondata)  # 추가된 부분
