...
# 지도 해상도, 맵 크기, 중심 좌표 등 초기 설정
params_map = {
    "MAP_RESOLUTION": 0.05,
    "OCCUPANCY_UP": 0.02,
    "OCCUPANCY_DOWN": 0.01,
    "MAP_CENTER" : (4.0, 8.0),
    # "MAP_CENTER" : (-42.0, -54.0),
    "MAP_SIZE": (350, 350),
    "MAP_FILENAME": 'test.png',
    "MAPVIS_RESIZE_SCALE": 2.0
}

# 두 점 사이의 직선 상의 픽셀(점유 상태) 찾기
...
class Mapping:
    ...
    def __init__(self, params_map):
        ...
        # 맵 사이즈 출력
        # print(f"Map Size: {self.map_size[0].astype(np.int)}, {self.map_size[1].astype(np.int)} (in pixels)")
        # print(f"Map Resolution: {self.map_resolution} meters/pixel")
    ...
    def update(self, pose, laser):
        ...
        # print(f"Updated map at position {pose[0]}, {pose[1]}")
    ...
    def save_map(self):
        # print(f"Saving map to {self.map_filename}")
        map_clone = self.map.copy()
        cv2.imwrite(self.map_filename, map_clone*255)
        # print("Map saved.")

    def show_pose_and_points(self, pose, laser_global):
        # print(f"Pose: {pose}, Laser Points: {laser_global.shape}")
        # 디버깅용: 변환된 포즈와 레이저 좌표 출력
        # print(f"Pose (transformed): {pose}")
        # print(f"Laser Points (global): {laser_global}")
        ...
        print(f"Pose Grid Coordinates: ({pose_x}, {pose_y})")
        # print(f"Laser Grid Coordinates: ({laser_global_x}, {laser_global_y})")
...
class Mapper(Node):
    ...
    def scan_callback(self,msg):
        ...
        # 초기 위치 출력
        self.get_logger().info(f'Initial Position: x={pose_x}, y={pose_y}, heading={heading}')
        ...
        # 디버깅 로그 추가
        # self.get_logger().info(f'Pose: {pose}, Laser Data: {laser}')
        self.mapping.update(pose, laser)
