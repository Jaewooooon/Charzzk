...
class Localization:
    ...
    def load_map(self):
        ...
        pkg_path = os.path.join(os.getcwd(), 'src')
        back_folder='sub1'
        folder_name='map'
        file_name='final_map.txt'
    ...
    def show_particle(self, particles, laser=None, gt_pose=None):
        print("show_particle 함수가 호출되었습니다!")
    ...
    def initialize_particles(self, pose=None, factor=None):
        print("initialize_particles 함수가 호출되었습니다.")
        rows, cols = self.map.shape
        ...
        print("particles initialized:", self.particles)
...
class Localizer(Node):
    ...
    def init_pose_callback(self,msg):
        if msg.header.frame_id in ['map', 'odom']:
            ...
            print("초기 위치가 설정되었습니다:", self.odom_x, self.odom_y, self.odom_theta)

    def turtlebot_status_callback(self, msg):

        # print("init????",self.is_init_pose)
        # print("imu????",self.is_imu)
