...
class Controller(Node):
    ...
    def __init__(self):
        print("Controller node is running", flush = True)
        super().__init__('controller')   #('sub1_controller')
        #self.get_logger().info("Controller node is running ")
        ## 메시지 송신을 위한 PUBLISHER 생성
        self.cmd_publisher = self.create_publisher(Twist, 'cmd_vel', 10)    # 터틀봇 제어(/ctrl_cmd)
        # self.app_control_pub = self.create_publisher(Int8MultiArray, 'app_control', 10) # 가전제어(/app_control)
    ...
    def listener_callback(self, msg):
        ...
        # print("now_linear_vel_inController: {}   now_angular_vel_inController: {} \n ".format(
    ...
    def turtlebot_go(self) :
        ...
        self.stop_timer = self.create_timer(5.0, self.turtlebot_stop)
        #self.cmd_publisher.publish(self.cmd_msg)
    ...
    def timer_callback(self):
        ...
        '''
        #print("현재 선속도: {} \n 현재 각속도: {} \n 배터리 상태: {} \n 충전 상태: {}".format(
        #    self.turtlebot_status_msg.twist.linear.x, self.turtlebot_status_msg.twist.angular.z, self.turtlebot_status_msg.battery_percentage, self.turtlebot_status_msg.power_supply_status))
