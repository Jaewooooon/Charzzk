...
# 기록을 해서 만드는 방법은 주행할 경로를 만드는 가장 쉬운 방법입니다. 생성된 텍스트 파일은 추 후에 텍스트 파일을 읽어서 Path 메시지를 publish 하는 노드(path_pub)에서 사용하고, 
# 경로 추종알고리즘에서 사용 됩니다.
...
class makePath(Node):
    ...
    def __init__(self):
        ...
        print("MakePath node is running", flush = True)
        # sself.get_logger().info("MakePath node is running")

        # 로직 1. 노드에 필요한 publisher, subscriber 생성      
        self.path_pub = self.create_publisher(Path, 'global_path', 10)
        self.subscription = self.create_subscription(Odometry,'/odom',self.listener_callback,10)    #  /odom 토픽을 구독하여 Odometry 메시지를 수신

        '''
        로직 2. 저장할 경로 및 텍스트파일 이름을 정하고, 쓰기 모드로 열기
        '''
        pkg_path = 'C:\\Users\\SSAFY\\Desktop\\ros2_ws\\src\\sub2'
        folder_name = 'path' 

        # 로봇충전소 -> 주차 위치 별 global path 만들어야 함
        file_name = 'globalPath.txt'
    ...
    def listener_callback(self,msg):
        print("listenerCallback in MakePath is running", flush = True)

        print('x_inMakePath : {} , y_inMakePath : {} '.format(msg.pose.pose.position.x,msg.pose.pose.position.y))
        if self.is_odom ==False :   
            ...
            if distance > 0.1 :
                ...
                waypint_pose.pose.orientation.w= yaw
                self.path_msg.poses.append(waypint_pose)
                self.path_pub.publish(self.path_msg) 
                '''v
                    로직 6. x,y 를 문자열로 바꾸고 x와 y 사이의 문자열은 /t 로 구함
                '''
                data= '{0}\t{1}\n'.format(x,y)
                self.f.write(data)
                # 이전 위치 -> 현재 위치로 업데이트
